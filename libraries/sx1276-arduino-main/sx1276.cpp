...
//ICACHE_FLASH_ATTR
void SX1276Radio::DirtyReadRegister(byte reg, byte& result)
{
  //SPI.beginTransaction(spi_settings_);
  digitalWrite(cs_pin_, LOW);
  SPI.transfer(reg);
...
bool SX1276Radio::Begin()
{
  ...
  // Modified by Cesar to bitwise operations so we do not alter reserved bit 4 and 5:
	/*
	Bits 0-2: Device modes
	000  SLEEP
	001  STDBY
	...
	Bit 3: 0=HF, 1=LF
	Bits 4-5: reserved (better leave as they are)
	Bit 6: 0=LoRa regs; 1=FSK regs in LoRa mode (should be = 0)
	Bit 7: 0=FSK/OOK mode; 1=LoRa mode (should be = 1)
	*/
  //WriteRegister(SX1276REG_OpMode, v & 0xf8, true);
  //WriteRegister(SX1276REG_OpMode, v & 0x80, true);
	/*
    v &=  ~1; //Bit 0 = 0
    v &=  ~2; //Bit 1 = 0
    v &=  ~4; //Bit 2 = 0
	v &=   8; //But 3 = 1
	//v &=  16; //Bit 4 unchanged
	//v &=  32; //Bit 5 unchanged
	//v &=  64; //Bit 6 unchanged
	//v &= 128; //Bit 7 unchanged
	WriteRegister(SX1276REG_OpMode, v, true);
	*/
	
  // Sleep: 1000 0000 = 0x80
  WriteRegister(SX1276REG_OpMode, SX1276_OPMODE_SLEEP, true);

  // LoRa, Standby: 1000 0001 = 0x81
  ...
  ReadRegister(SX1276REG_OpMode, v);
  // Changed to a bitwise testing since reserved bits can vary:
  if (!(v & 1)) {
...
bool SX1276Radio::TransmitMessage(const void *payload, byte len, bool withStandby)
{
  ...
  elapsedMillis t0;
#endif
DEBUG ("TransmitMessage(const void *payload=%s, byte len=%02X, bool withStandby)\n\r", payload, len);
  ...
  if (cached_tx_payload_length_ != len || !tx_warm_) {
	DEBUG ("cached_tx_payload_length_ (%02X) != len (%02X)\n\r", cached_tx_payload_length_, len);
    cached_tx_payload_length_ = len;
    cached_tx_toa_ = PredictTimeOnAir(len)*10; // Cesar
  ...
  // Setting FIFO without offsetting it:
  DEBUG("*** DoRegister(SX1276REG_FifoAddrPtr=%02X, FIFO_START=%02x); ***\n\r", SX1276REG_FifoAddrPtr, FIFO_START);
  //DoRegister(SX1276REG_FifoAddrPtr + SX1276_SPI_WRITE_MASK, FIFO_START);
  DoRegister(SX1276REG_FifoAddrPtr, FIFO_START);
  delayMicroseconds(SPI_CS_DELAY_US);
  
  //v = DoRegister(SX1276REG_FifoAddrPtr, 0); // CESAR: this was the original code
  DEBUG("*** 1st DirtyReadRegister(SX1276REG_FifoAddrPtr=%02x, v); ***\n\r", SX1276REG_FifoAddrPtr);
  // Dirty read read data ignoring SPI transaction mode
  DirtyReadRegister(SX1276REG_FifoAddrPtr, v);
  delayMicroseconds(SPI_CS_DELAY_US);
  DEBUG("*** 1st DirtyReadRegister(SX1276REG_FifoAddrPtr) got v=%02x ***\n\r", v);

  
  DEBUG("*** Entering for loop for 0 to %d (0x%02X)***\n\r", len - 1, len - 1);
  for (byte n=0; n < len; n++) {
	// Cesar: let's not add SX1276_SPI_WRITE_MASK to FiFo
    //DoRegister(SX1276REG_Fifo + SX1276_SPI_WRITE_MASK, ((const byte *)payload)[n]);
	DEBUG ("    > DoRegister(SX1276REG_Fifo=%02X, (payload)[n=%02X]=%02X);\n\r", SX1276REG_Fifo, n, ((const byte *)payload)[n]);
    DoRegister(SX1276REG_Fifo, ((const byte *)payload)[n]);
    delayMicroseconds(SPI_CS_DELAY_US);
  }
  DEBUG("*** Exited for loop ***\n\r");

  //v = DoRegister(SX1276REG_FifoAddrPtr, 0); // CESAR: this was the original code
  //DEBUG("*** 2nd DirtyReadRegister(SX1276REG_FifoAddrPtr=%02x, v); ***\n\r", SX1276REG_FifoAddrPtr);
  // Dirty read read data ignoring SPI transaction mode
  DirtyReadRegister(SX1276REG_FifoAddrPtr, v);
  delayMicroseconds(SPI_CS_DELAY_US);
  DEBUG("*** 2nd DirtyReadRegister(SX1276REG_FifoAddrPtr=%02X) got v=%02x ***\n\r", SX1276REG_FifoAddrPtr, v);


  // Cesar: Changed to set OPMODE_TX without messing with OPMODE + WriteMask:
  // DEBUG("*** DoRegister(%02X + %02x, %02x) ***\n\r", SX1276REG_OpMode, SX1276_SPI_WRITE_MASK, SX1276_OPMODE_TX);
  // DoRegister(SX1276REG_OpMode + SX1276_SPI_WRITE_MASK, SX1276_OPMODE_TX);
  // delayMicroseconds(SPI_CS_DELAY_US);
  DEBUG("*** DoRegister(SX1276REG_OpMode=%02X, SX1276_OPMODE_TX=%02x) ***\n\r", SX1276REG_OpMode, SX1276_OPMODE_TX);
  DoRegister(SX1276REG_OpMode, SX1276_OPMODE_TX);
  delayMicroseconds(SPI_CS_DELAY_US);
  ...
  DEBUG("*** Entering do-while (TIME-OUT) loop ***\n\r");

  v = 0;
  do {
    DirtyReadRegister(SX1276REG_IrqFlags, v);
    if (v & 8) {
      ok = true;
      break;
    }
...
*/
  ...
  delayMicroseconds(SPI_CS_DELAY_US);
  DEBUG("*** Exit do-while (TIME-OUT) loop ***\n\r");
/*  
  DEBUG("*** DoRegister(SX1276REG_IrqFlags=%02x + SX1276_SPI_WRITE_MASK=%02x, 0xff);***\n\r", SX1276REG_IrqFlags, SX1276_SPI_WRITE_MASK);
  DoRegister(SX1276REG_IrqFlags + SX1276_SPI_WRITE_MASK, 0xff);
  delayMicroseconds(SPI_CS_DELAY_US);
*/  
  DEBUG("*** DoRegister(SX1276REG_IrqFlags=%02x, MASK=0xff);***\n\r", SX1276REG_IrqFlags);
  DoRegister(SX1276REG_IrqFlags, 0xff);
  delayMicroseconds(SPI_CS_DELAY_US);
