// This code run in the slave mode,receive the tx packet and send reply to master
...
// tx packet
unsigned char  txbuf[payload_length]={'r','e','t','u','r','n','_','t','e','s','t'};
// rx packet
	...
	{			
		...
		{				
			...
			{ 
				...
				if(strstr(rxbuf,"send_test")!=NULL) 		// If the received data is “send_test”
				{
					LED_RX=LED_RX^1;			// data is correct then send the reply
					Flag.is_tx=1;
					tx_mode();				
				}
				else
...
void tx_mode (void) 
{
	...
	SPIWriteReg(LR_RegIrqFlags,0xff);			// 清中断
	SPIWriteReg(LR_RegOpMode,0x09);  			// 进入standby模式
