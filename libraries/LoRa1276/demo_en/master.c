...
void main(void)
{
	...
	count_tx = 0;
	Flag.reach_tx = 0;	// initialize tx flag

	while(1)
	{
		if(Flag.reach_tx)						// transmit packet on time, and wait for the reply
		{
			Flag.reach_tx = 0;
		...
		{				
			...
			{ 
				...
				if(strstr(rxbuf,"return_test")!=NULL) 		// check the packet
				{
					LED_RX=LED_RX^1;			// data is correct then flash the LED
					Flag.is_tx=1;
					SPIWriteReg(LR_RegOpMode,0x09);  	// ??Standby??
				}
				else
				{
...
void SX1276_Config(void)
{
	...
	SPIWriteReg(LR_RegFrLsb,0x00);		// frequency£º434Mhz					 
...
void tx_mode (void) 
{
	...
	SPIWriteReg(LR_RegPayloadLength,payload_length);	// packet length

	addr = SPIReadReg(LR_RegFifoTxBaseAddr);		// read TxBaseAddr        
	SPIWriteReg(LR_RegFifoAddrPtr,addr);			// TxBaseAddr->FifoAddrPtr          
	
	SPIBurstWrite(0x00,txbuf,payload_length);		// fill data into fifo
	...
	SPIWriteReg(LR_RegIrqFlags,0xff);			// clear interrupt
	SPIWriteReg(LR_RegOpMode,0x09);  			// enter standby
